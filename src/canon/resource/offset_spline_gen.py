...
class BoundsCreator(object):
    ...
    def __init__(self) -> None:
        ...
        for _point in original_spline_file:
            _points.append([float(_point[0]), float(_point[1]), float(_point[2])])
        _px = np.array([_point[0] for _point in _points])
        _py = np.array([_point[1] for _point in _points])
        _pv = np.array([_point[2] for _point in _points])
        ...
        for i in range(len(_ox)):  offset_spline_file.writerow([_ox[i], _oy[i], _pv[i] + OFFSET_VEL])
